...
import {CategoriesProduct, Product, ProductSizeQuantity, Size, Stock, User, UserStock} from "@/models";
...
export default function Products({user, stocks, products, categories, sizes}: {
    user: any,
    stocks: any,
    products: any,
    categories: any,
    ...
    const handleCreateProduct = async (e: any) => {
        ...
        // Création du produit
        try {
            const newProduct = await DataStore.save(new Product({
                label: product.label,
                // @ts-ignore
                categoriesproductID: product.categoryID,
            ...
            for (const size of product.sizes) {
                const sizeDb = await DataStore.save(new ProductSizeQuantity({
                    productID: productID,
                    sizeID: size.id,
    ...
    const handleFilterSize = () => {
        ...
        const newSizeTab = sizesTab.filter((size: any) => size.categoriesproductID === activeCategory);
        console.log(newSizeTab);
        // @ts-ignore
    ...
    return (
        <Layout stocks={stocks}>
            ...
            <CustomModal isOpen={isOpen} onClose={onClose} title={"Création d'un nouveau produit"}>
                <form onSubmit={handleCreateProduct}>
                    <Box padding={2}>
                        <Input onChange={handleChange} margin="10px 0" name="label" placeholder={"Nom du produit"}/>
                        <Select onChange={(e) => {
                            handleChange(e);
                        }} margin="10px 0" name="categoryID" placeholder={"Choisir une catégorie"}>
                        ...
                        <Box padding={4}>
                            ...
                            {product.sizes.map((size, index) => (
                                <Flex key={index} margin={"10px 0"}>
                                    <Select onChange={(e: any) => handleChange(e, index)} name="id"
                                            placeholder="Choisir une taille">
                                        {listSizes.map((size: any) => (
                                            <option key={size.id} value={size.id}>{size.label}</option>
                                        ))}
                                    </Select>
                                    <Input name="quantity" onChange={(e: any) => handleChange(e, index)}
                        ...
                        <Button onClick={handleAddSize} colorScheme="gray">Ajouter une taille</Button>
                    </Box>
                    <Flex margin={"20px 0"}>
                        <Button onClick={() => {
                            setIsLoading(false);
                            onClose();
                        }} variant="link" marginRight={"20px"}
                                colorScheme="gray">Annuler</Button>
                        <Button isLoading={isLoading} loadingText={"Création du produit en cours"} type="submit"
                                colorScheme="green">Créer mon produit</Button>
                    </Flex>
                </form>
            </CustomModal>
        </Layout>
...
export async function getServerSideProps({req}: { req: any }) {
    ...
    const products = await DataStore.query(Product, (p: any) => p.stockID.eq(userDb[0].activeStockID));
